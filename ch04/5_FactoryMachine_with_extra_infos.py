...
# load the u.user file info a dataframe
u_cols = ['user_id', 'age', 'sex', 'occupation', 'zip_code']
users = pd.read_csv('./u.user', sep='|', names=u_cols, encoding='latin-1')

# load the u.item file info a dataframe
...
# load the u.data file info a dataframe
...
genre = ['unknown', 'Action', 'Adventure', 'Animation', 'Children\'s', 'Comedy',
          ...
          'Musical', 'Mystery', 'Romance', 'Sci-Fi', 'Thriller', 'War', 'Western']
for i in genre:
    genre_dict[i] = start_point + len(gender_dict)
...
print('ratings, movies merged shape', x.shape)
x = pd.merge(x, users, how='outer', on='user_id')
print('x, users merged shape', x.shape)
...
for i in range(len(x)):
    ...
    x_index.append(user_dict[case['user_id']])  # user id one hot encoding
    x_value.append(1)
    x_index.append(item_dict[case['movie_id']])  # movie id one hot encoding
    x_value.append(1)
    x_index.append(occ_dict[case['occupation']])  # occupation id encoding
    x_value.append(1)
    x_index.append(gender_dict[case['sex']])  # occupation id encoding
    x_value.append(1)
...
K = 200
fm1 = FM(num_x, K, data, y, alpha=0.00005, beta=0.002, train_ratio=0.75, iterations=900,
         tolerance=0.0001, l2_reg=True, verbose=True)
fm1.test()
