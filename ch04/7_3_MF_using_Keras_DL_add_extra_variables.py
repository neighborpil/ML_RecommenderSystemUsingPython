...
occupation = {}  # key: 직업명, value: 인덱스


def convert_occ(x):
    if x in occupation:  # occupation에 index가 있다면 그 인덱스 반환
        return occupation[x]
    else:  # occupation에 index가 없다면 occupation에 인덱스 추가 후 그 인덱스 반환
        occupation[x] = len(occupation)
        return occupation[x]


users['occupation'] = users['occupation'].apply(convert_occ)  # string을 integer로 변환
...
# add occupation to model
occ = Input(shape=(1, ))
occ_embedding = Embedding(L, 3, embeddings_regularizer=l2())(occ) # 3개의 잠재요인
